...
exports.createexcursion = async (req, res, next) => {
  ...
  if (req.body.name == "" || req.body.name == undefined) {
    ...
    res
      .status(400)
      .json({ Message: "Excursion description missing :: Enter Again" });
  } else if (
    req.body.lat == "" ||
    ...
    res
      .status(400)
      .json({ Message: "Please provide both latitide and longitude.." });
  } else if (req.body.date == "" || req.body.date == undefined) {
    //it is not a date with format YYYY-MM-DD
    res
      .status(400)
      .json({ Message: "Please provide Date in format YYYY-MM-DD" });
    ...
    res
      .status(400)
      .json({ Message: "Please provide valid latitude (-90 to 90)" });
  } else if (!isLongitude(req.body.lng)) {
    res
      .status(400)
      .json({ Message: "Please provide valid longitude. (-180 to 180)" });
  } else {
    await User.findOne({
      where: {
        [Op.and]: [{ email: req.user.email }, { role: "admin" }],
      },
    }).then((resp) => {
      if (!resp) {
        res.status(400).json({
          Message: "You are not Authorized for this operation, Contact Admin",
        });
        return;
      } else {
  ...
  await User.findOne({
    ...
    if (!resp) {
      ...
      if (req.body.lat && req.body.lng) {
        ...
        const myDistance = 80000; // e.g. 10 kilometres
              ...
              include: [
                [
                  Sequelize.fn(
                    ...
                    Sequelize.fn(
                      "ST_SetSRID",
                      Sequelize.fn("ST_MakePoint", req.body.lat, req.body.lng),
                      4326
                    )
                  ),
                  "distance",
        ...
        res
          .status(400)
          .json({ Message: "Please provide both Lat and Lng to search" });
        return;
      }
    }
  });
